...
//    doc file
    ...
    public ArrayList<KhachHang> readKhachHangFromFile(String fileName){
...
//         public KhachHang(int maKH ,String tenKH, String sdt, int soCMND)
        KhachHang khachHang = new KhachHang(Integer.parseInt(datas[0]), datas[1]
                , datas[2], Integer.parseInt(datas[3]));
        return khachHang;
    }
    public ArrayList<Phong> readPhongFromFile(String fileName){
        openFileToRead(fileName);
...
//                +phong.getDienTich()+"|"+phong.getGiaThue());
    ...
    public ArrayList<LoaiPhong> readLoaiPhongFromFile(String fileName){
        openFileToRead(fileName);
...
//                +loaiPhong.getGhiChu());
        LoaiPhong loaiPhong = new LoaiPhong(Integer.parseInt(datas[0]),datas[1],datas[2]);
        return loaiPhong;
    }

    public ArrayList<ThuePhong> readThuePhongFromFile(String fileName) throws ParseException {
        openFileToRead(fileName);
...
//                +"|"+thuePhong.getNgayRa()+"|"+thuePhong.getDatCoc());
    ...
    public void writeThanhToanToFile(ThanhToan thanhToan, String fileName){
        ...
        printWriter.println(thanhToan.getThuePhong().getKhachHang().getMaKH()+"|"+thanhToan.getThanhTien()+"|"
                +thanhToan.getHinhThucThanhToan()+"|"+thanhToan.getGhiChu()+"|"+thanhToan.getNgayThanhToan());
        try {
            closeFileAfterWrite(fileName);
        } catch (IOException e) {
            e.printStackTrace();
